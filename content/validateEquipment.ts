...
const SLOT           = [
  ...
  'main_hand', 'off_hand', 'necklace', 'ring', 'earring',
] as const;
...
export const ItemSchema = z
  .object({
    slug:           z.string().regex(/^[a-z0-9_-]+$/, 'must be lowercase (a-z, 0-9, -, _)'),
    name:           z.string().min(1),
  ...
  .superRefine((item, ctx) => {
    ...
    if (isMainHand) {
      ...
      if (item.geometryShape !== null || item.geometryWidth !== null || item.geometryRange !== null) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ['geometryShape'],
...
function crossCheck(items: ItemDef[]): string[] {
  ...
  for (const item of items) {
    ...
    if (item.armorWeight !== null && (item.slot === 'main_hand' || item.slot === 'off_hand')) {
      errors.push(`${item.slug}: armorWeight must be null for ${item.slot} items (weapons/focuses aren't cloth/chain/plate)`);
    }
    ...
    if (item.rarity === 'epic' && maxStat < 20) {
      errors.push(`${item.slug}: epic item must have at least one stat >= 20 (max found: ${maxStat})`);
    }
    if (item.rarity === 'legendary' && maxStat < 40) {
      errors.push(`${item.slug}: legendary item must have at least one stat >= 40 (max found: ${maxStat})`);
    }

    if (item.stats.moveSpeed !== undefined && item.stats.moveSpeed >= 0.5) {
      errors.push(`${item.slug}: moveSpeed is a multiplier and must be < 0.5 (got ${item.stats.moveSpeed})`);
    }
    if (item.stats.evasion !== undefined && item.stats.evasion >= 0.20) {
      errors.push(`${item.slug}: evasion is capped at 0.20 per item (got ${item.stats.evasion})`);
    }
    ...
    if (item.slot === 'main_hand' && item.geometryWidth !== null && item.geometryWidth > 1.0) {
      const totalDamage =
...
export function validateEquipment(items: unknown[]): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
  const valid: ItemDef[] = [];
