...
export const CardSchema = z
  .object({
    slug:            z.string().regex(/^[a-z0-9-]+$/, 'must be lowercase-hyphenated (a-z, 0-9, -)'),
    name:            z.string().min(1),
...
function crossCheck(cards: CardDef[]): string[] {
  ...
  for (const card of cards) {
    if (card.rarity === 'legendary' && card.minLevel < 35) {
      errors.push(`${card.slug}: legendary card must have minLevel >= 35 (got ${card.minLevel})`);
    }
    if (card.rarity === 'epic' && card.minLevel < 20) {
      errors.push(`${card.slug}: epic card must have minLevel >= 20 (got ${card.minLevel})`);
    }
    if (card.type === 'passive' && card.passiveKind === 'ward' && card.mpCost !== 0) {
      errors.push(`${card.slug}: ward passive must have mpCost === 0 (got ${card.mpCost})`);
    }
  }
...
export function validateCards(cards: unknown[]): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
  const valid: CardDef[] = [];
