...
This is a live,
level-up-triggered event, independent of `CON-TMP-015`'s death-gated
...
moment during play, not at death — conflating it with `CON-TMP-015`'s
...
`SW-TMP-015`, and correctly never re-shows the message, since there is no
...
- [CON-TMP-015](CON-TMP-015-tutorial-completion-stamps-on-death-once-the-characters-zone-appropriate-max-level-was-reached.md) — the separate, death-gated account milestone this message does not depend on
- [SW-TMP-012](SW-TMP-012-xp-grant-returns-zero-at-or-above-the-zones-max-level.md) — the same threshold this message's trigger reads
