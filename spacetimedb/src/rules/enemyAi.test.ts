  ...
  () => {
    describe('decideChasing', () => {
      it('reports targetLost when the target is null', () => {
        expect(decideChasing(0, 0, null).kind).toBe('targetLost');
      });

      it('reports targetLost when the target is dead', () => {
        expect(
          decideChasing(0, 0, { posX: 10, posY: 0, alive: false }).kind,
        ).toBe('targetLost');
      });

      it('reports deaggro when the target is beyond DEAGGRO_RANGE', () => {
        const decision = decideChasing(0, 0, {
  ...
  () => {
    describe('decideCooldown', () => {
      it('reports targetLost when the target is null or dead', () => {
        expect(decideCooldown(0, 0, null, 0n, null, 1).kind).toBe('targetLost');
        expect(
          decideCooldown(0, 0, { posX: 0, posY: 0, alive: false }, 0n, null, 1)
            .kind,
        ).toBe('targetLost');
      });
  ...
  () => {
    describe('decideResetting', () => {
      it('steps toward spawn and heals when nothing re-aggroes', () => {
        const decision = decideResetting(100, 0, 0, 0, 50, 100, []);
      ...
      it('reports arrived at full HP once it reaches its exact spawn position', () => {
        ...
        const decision = decideResetting(step, 0, 0, 0, 90, 100, []);
      ...
      it('re-aggroes mid-reset if a candidate comes into range', () => {
        ...
        const decision = decideResetting(100, 0, 0, 0, 50, 100, [target]);
      ...
      it('never heals past maxHp', () => {
        const decision = decideResetting(100, 0, 0, 0, 100, 100, []);
...
verifies(
  ...
  () => {
    describe('CHASE_SPEED stays below the player base move speed', () => {
      it('CHASE_SPEED (px/s) is strictly less than the player base move speed (180px/s, see docs/BALANCE.md)', () => {
        ...
        expect(CHASE_SPEED).toBeLessThan(PLAYER_BASE_MOVE_SPEED_PX_S);
      });
    });
  },
);
